...
def check_captcha(message):
    ...
    # 1. Проверяем, есть ли пользователь в списке ожидания
    if user_id not in pending_captcha:
        # Если его нет (например, таймер кикнул его), просто удаляем сообщение
        try:
            bot.delete_message(chat_id, message.message_id)
        except Exception:
            pass # Не страшно, если не вышло
        return

    chat_id_from_dict, answer = pending_captcha[user_id]

    # 2. В ЛЮБОМ СЛУЧАЕ удаляем сообщение пользователя (ваше 1-е требование)
    try:
        bot.delete_message(chat_id, message.message_id)
    except Exception as e:
    ...
    is_correct = False
    try:
        # 3. Проверяем, является ли ответ правильным
        if int(message.text.strip()) == answer:
            is_correct = True
    except ValueError:
        # Это не число или ошибка, is_correct остается False
        pass

    # 4. Реагируем в зависимости от ответа
    if is_correct:
        # 4A. ПРАВИЛЬНЫЙ ОТВЕТ
        del pending_captcha[user_id]
        try:
            ...
            # Отправляем временное приветствие
            welcome_msg = bot.send_message(
                chat_id, f"✅ Добро пожаловать, {message.from_user.first_name}!")

            # Удаляем приветствие через 5 секунд (чтобы не мусорить)
        ...
        # 4B. НЕПРАВИЛЬНЫЙ ОТВET (ваше 2-е требование: Микро-мут)
        try:
            # Выдаем ПОЛНЫЙ мут (can_send_messages=False) на 4 секунды
            # Telegram сам вернет пользователя в состояние "только текст" по истечении
            bot.restrict_chat_member(
                chat_id,
                user_id,
                can_send_messages=False, # Полный мут
            )
